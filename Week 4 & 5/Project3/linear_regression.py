...
class LinearRegression(analysis.Analysis):
    ...
    def r_squared(self, y_pred):
        ...
        R2: float.
            The R^2 statistic
        """
        return 1 - ((self.compute_residuals(y_pred) ** 2).sum()/((self.y - self.y.mean()) ** 2).sum())
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), hists_on_diag=True):
        ...
        for row in range(len(data_vars)):
            for col in range(len(data_vars)):
                ...
                x = self.select_variable(data_vars[col])
                xline = np.linspace(x.min(), x.max()).squeeze()
                yline = (self.intercept + xline * self.slope).squeeze()
                axs[row, col].plot(xline, yline)
                axs[row, col].set_title(f"R^2={self.R2:0.4f}")
                if hists_on_diag and row == col:
                    numVars = len(data_vars)
                    axs[row, col].remove()
                    axs[row, col] = fig.add_subplot(numVars, numVars, row * numVars + col + 1)
                    if col < numVars - 1:
                        axs[row, col].set_xticks([])
                    else:
                        axs[row, col].set_xlabel(data_vars[row])
                    if row > 0:
