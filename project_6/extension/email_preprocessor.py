    ...
    for path in [f"{email_path}spam/", f"{email_path}ham/"]:
        for email in os.listdir(path):
            if email.endswith(".txt"):
                num_emails += 1
                words = tokenize_words(open(path+email).read())
                for word in words:
...
def find_top_words(word_freq, num_features=200):
    ...
    top_words = [item[0] for item in sorted_word_freq[:num_features]]
    counts = [item[1] for item in sorted_word_freq[:num_features]]
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron/'):
    ...
    '''
    feats = np.ndarray(shape=(num_emails, len(top_words)))
    y = np.ndarray(shape=(num_emails,))
    path2class = {f"{email_path}ham/": 0, f"{email_path}spam/": 1}
    row = 0
    ...
    for path in path2class.keys():
        for email in os.listdir(path):
            ...
            for i in range(len(top_words)):
                feats[row, i] = words.count(top_words[i])
            y[row] = path2class[path]
            row += 1
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    inds_train = inds[: int(inds.shape[0] * (1 - test_prop))]
    inds_test = inds[inds_train.shape[0]:]
